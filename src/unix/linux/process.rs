        ...
        match fs::read_to_string(&limits_files) {
            Ok(content) => {
                for line in content.lines() {
                    let mut parts = line.split_whitespace().filter(|p| !p.is_empty());
                    if parts.next() == Some("Max open files") {
                        return parts.next().and_then(|nb| u32::from_str(nb).ok());
                    }
                }
                None
            }
            Err(_error) => {
                sysinfo_debug!("Failed to get limits in `{}`: {_error:?}", limits_files.display());
                None
            }
        }
    }
